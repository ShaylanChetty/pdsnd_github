...
def get_filters():
    ...
    while True:
        ...
        month = input(
            "\n Choose a month (January, February, March, April, May, June)? Type 'None' for no month filter\n").title()
        if month in months:
            break
        else:
    ...
    while True:
        ...
        day = input(
            "\n Choose a day? (Monday, Tuesday, Wednesday, Thursday, Friday, Saturday, Sunday)? Type 'None' for no day filter \n").title()
        if day in days:
            break
        else:
